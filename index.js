...
async function getDealsHandler(req, res) {
  try {
    ...
    // Get all deals that have not been claimed by the specified user
    const result = await client.query(
      `SELECT d.*, u.Name AS UserName
       FROM Deals d
       LEFT JOIN Users u ON d.User_ID = u.ID
       LEFT JOIN ClaimedDeals cd ON d.ID = cd.Deal_ID AND cd.User_ID = $1
       WHERE (d.Status = 'Active' OR d.Status = 'Expired' OR d.Status = 'Inactive')
